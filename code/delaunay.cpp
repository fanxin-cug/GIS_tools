...
*/
void delaunay_free_halfedges( delaunay_t *del )
{
	unsigned int		i;
	halfedge_t		*d, *sig;
...
*/
static int delaunay_classify_point( halfedge_t *d, point2d_t *pt )
{
	point2d_t		*s, *e;
...
*/
static int delaunay_init_seg( delaunay_t *del, int start )
{
	halfedge_t		*d0, *d1;
	point2d_t		*pt0, *pt1;
...
*/
static int delaunay_init_tri( delaunay_t *del, int start )
{
...
*/
static void delaunay_remove_edge( halfedge_t *d )
{
...
*/
static halfedge_t* delaunay_valid_left( halfedge_t* b )
{
	point2d_t		*g, *d, *u, *v;
	halfedge_t		*c, *du, *dg;
...
*/
static halfedge_t* delaunay_valid_right( halfedge_t *b )
{
	point2d_t		*rv, *lv, *u, *v;
	halfedge_t		*c, *dd, *du;
...
* validate a link
*/
static halfedge_t* delaunay_valid_link( halfedge_t *b )
{
	...
	g	= b->vertex;
	gd	= delaunay_valid_left(b);
	g_p	= gd->vertex;

	assert(b->pair);
	d	= b->pair->vertex;
	dd	= delaunay_valid_right(b);
	d_p	= dd->vertex;
	assert(b->pair);
...
*/
static halfedge_t* delaunay_get_lower_tangent( delaunay_t *left, delaunay_t *right )
{
	point2d_t	*pl, *pr;
...
*/
static void delaunay_link( delaunay_t *result, delaunay_t *left, delaunay_t *right )
{
	point2d_t		*u, *v, *ml, *mr;
	halfedge_t		*base;
	...
	base		= delaunay_get_lower_tangent(left, right);

	u		= base->next->pair->vertex;
	v		= base->pair->prev->pair->vertex;

	while( delaunay_classify_point(base, u) == ON_LEFT ||
	       delaunay_classify_point(base, v) == ON_LEFT )
	{
		base	= delaunay_valid_link(base);
		u	= base->next->pair->vertex;
		v	= base->pair->prev->pair->vertex;
	}
	...
	while( delaunay_classify_point( right->rightmost_he, right->rightmost_he->prev->pair->vertex ) == ON_RIGHT )
	       right->rightmost_he	= right->rightmost_he->prev;

	while( delaunay_classify_point( left->leftmost_he, left->leftmost_he->prev->pair->vertex ) == ON_RIGHT )
...
*/
void delaunay_divide_and_conquer( delaunay_t *del, int start, int end )
{
	...
	if( n > 3 ) {
		...
		delaunay_divide_and_conquer( &left, start, start + i - 1 );
		delaunay_divide_and_conquer( &right, start + i, end );
		delaunay_link( del, &left, &right );
	} else {
		if( n == 3 ) {
			delaunay_init_tri( del, start );
		} else {
			if( n == 2 ) {
				delaunay_init_seg( del, start );
			}
		}
	}
}
...
*/
void delaunay_build_faces( delaunay_t *del )
{
	unsigned int	i;
	halfedge_t	*curr;
...
/*
*/
delaunay2d_t* delaunay2d_from(delaunay_point2d_t *points, unsigned int num_points) {
	...
	if( num_points >= 3 ) {
		delaunay_divide_and_conquer( &del, 0, num_points - 1 );

		delaunay_build_faces( &del );
	...
	res->points	= (delaunay_point2d_t*)malloc(sizeof(delaunay_point2d_t) * num_points);
	assert( NULL != res->points );
	memcpy(res->points, points, sizeof(delaunay_point2d_t) * num_points);
...
tri_delaunay2d_t* tri_delaunay2d_from(delaunay2d_t* del) {
	...
	tdel->points		= (delaunay_point2d_t*)malloc(sizeof(delaunay_point2d_t) * del->num_points);
	assert( NULL != tdel->points );
	memcpy(tdel->points, del->points, sizeof(delaunay_point2d_t) * del->num_points);
